...
function dedupeLatestByPeerAndTarget(entries) {
  const byKey = new Map()
  for (const e of entries) {
    const key = `${e.peerId}:${e.targetTs}`
...
// Reactions are one-per-peer-per-message: reacting again with a different
// emoji replaces your previous one, and reacting with the same emoji again
// removes it (sent as emoji: null) -- so only each peer's LATEST reaction
// to a given message counts, same one-vote-per-peer principle used for the
// current score consensus above. Returns a plain object (not a Map, so
// it's trivially IPC/JSON safe): { [targetTs]: [{ peerId, emoji }, ...] }.
function getReactions(entries) {
  const latest = dedupeLatestByPeerAndTarget(entries.filter((e) => e.type === 'reaction'))
  const byTarget = {}
  for (const r of latest) {
    if (!r.emoji) continue
    if (!byTarget[r.targetTs]) byTarget[r.targetTs] = []
    byTarget[r.targetTs].push({ peerId: r.peerId, emoji: r.emoji })
  }
  return byTarget
}
