...
class Room extends EventEmitter {
  ...
  async join() {
    ...
    // Typing indicators are ephemeral and don't belong in the permanent
    // log, so they ride a Hypercore protocol extension instead of an
    // appended entry -- a ready made side channel over the exact same
    // replication connections, broadcasting live to whoever's currently
    // connected with nothing persisted anywhere.
    this.typingExt = this.localCore.registerExtension('terrace-typing', {
      encoding: 'json',
      onmessage: (message) => this.emit('typing', message)
    })
    ...
    this.swarm.on('connection', (socket, info) => {
      ...
      // Extensions are registered per-core, not per-connection: a peer's
      // typing broadcast travels over THEIR core's replication channel, so
      // we only receive it if we've registered the same extension name on
      // our read-only view of their core too, not just on our own.
      remoteCore.registerExtension('terrace-typing', {
        encoding: 'json',
        onmessage: (message) => this.emit('typing', message)
      })
  ...
  sendChat(text, replyToTs) {
    return this._append({ type: 'chat', text, replyToTs: replyToTs || null })
  }

  // emoji: null removes your reaction. Reacting again with a different
  // emoji replaces your previous one -- one active reaction per peer per
  // message, same as everything else here.
  react({ targetTs, emoji }) {
    return this._append({ type: 'reaction', targetTs, emoji: emoji || null })
  }
  ...
  // Ephemeral, never touches the log: broadcasts live to whoever is
  // currently connected over the same replication connections.
  setTyping(isTyping) {
    if (this.typingExt) this.typingExt.broadcast({ peerId: this.peerId, typing: !!isTyping })
  }
  ...
  getReactions() {
    return lb.getReactions(this.entries)
  }
  ...
  async close() {
    if (this.typingExt) this.typingExt.destroy()
